...
class DeviceLocationTracker(
    ...
    private fun initializeLocationProviders(): android.location.Location? {
    ...
    internal fun initializeProvidersIfHasPermission() =
        CoroutineScope(Dispatchers.IO + Job())
            .launch {
                launch {
                    currentAddressLine = String()
                    while (currentAddressLine == String()) {
                        if (isInternetConnection().first()) {
                            ...
                            deviceLocation?.let { onLocationChanged(it) }
                        }
                        delay(10_000)
                    }
                    this.cancel()
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    if (isInternetConnection().first()) {
                        ...
                        while (currentAddressLine == "") {
                            initializeProvidersIfHasPermission()
